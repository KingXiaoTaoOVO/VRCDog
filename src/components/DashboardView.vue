...
<template>
  <div class="h-full flex flex-col p-6 bg-surface-hover rounded-3xl relative overflow-hidden">
    ...
    <header class="mb-8 flex justify-between items-end shrink-0 z-10">
      <div>
        <h1 class="text-3xl font-extrabold text-text tracking-tight flex items-center gap-3">
          {{ t('dashboard.title') }}
          <span class="inline-flex items-center justify-center p-2 bg-primary/10 rounded-2xl shadow-sm border border-primary/20">
      ...
      <button
        :disabled="loading"
        class="px-5 py-2.5 bg-surface backdrop-blur rounded-full text-text-muted font-bold border border-border-soft shadow-sm hover:shadow-md hover:bg-surface hover:text-primary transition-all flex items-center gap-2 disabled:opacity-50"
        @click="fetchData"
      >
        <Zap
          class="w-4 h-4"
          :class="{'animate-pulse text-primary': loading}"
        /> {{ t('dashboard.refresh') }}
      </button>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 shrink-0 z-10">
      <!-- Online Friends Card -->
      <div class="bg-surface backdrop-blur-xl border border-white/80 rounded-3xl p-6 shadow-lg shadow-slate-200/40 hover:-translate-y-1 transition-all group cursor-default relative overflow-hidden">
        <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          <Users class="w-24 h-24 text-primary transform translate-x-4 -translate-y-4" />
        </div>
        <div class="flex items-center justify-between mb-4 relative z-10">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5 text-primary shadow-sm border border-primary/20">
            <Users class="w-6 h-6" />
          </div>
        </div>
        <div class="relative z-10">
          <p class="text-sm font-bold text-text-muted mb-1">
            {{ t('dashboard.online_friends') }}
          </p>
          <h3 class="text-3xl font-black text-text flex items-baseline gap-1">
            {{ onlineFriendsCount }} <span class="text-sm font-bold text-border-strong">{{ t('dashboard.unit_people') }}</span>
          </h3>
        </div>
      </div>
      ...
      <div class="bg-surface backdrop-blur-xl border border-white/80 rounded-3xl p-6 shadow-lg shadow-slate-200/40 hover:-translate-y-1 transition-all group cursor-default relative overflow-hidden">
        <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          <Map class="w-24 h-24 text-primary transform translate-x-4 -translate-y-4" />
        </div>
        <div class="flex items-center justify-between mb-4 relative z-10">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5 text-primary shadow-sm border border-primary/20">
            <Map class="w-6 h-6" />
        ...
        <div class="relative z-10">
          ...
          <h3 class="text-3xl font-black text-text flex items-baseline gap-1">
            {{ activeInstancesCount }} <span class="text-sm font-bold text-border-strong">{{ t('dashboard.unit_count') }}</span>
          </h3>
        </div>
      </div>
      ...
      <div class="bg-surface backdrop-blur-xl border border-white/80 rounded-3xl p-6 shadow-lg shadow-slate-200/40 hover:-translate-y-1 transition-all group cursor-default relative overflow-hidden">
        <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
          <Clock class="w-24 h-24 text-primary transform translate-x-4 -translate-y-4" />
        </div>
        <div class="flex items-center justify-between mb-4 relative z-10">
          <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5 text-primary shadow-sm border border-border-soft">
            <Clock class="w-6 h-6" />
          </div>
        </div>
        <div class="relative z-10">
          ...
          <h3 class="text-3xl font-black text-text flex items-baseline gap-1">
            {{ t('dashboard.latest') }} <span class="text-sm font-bold text-border-strong">{{ t('dashboard.status') }}</span>
          </h3>
        </div>
      </div>
      ...
      <div class="bg-surface backdrop-blur-xl border border-white/80 rounded-3xl p-6 shadow-lg shadow-slate-200/40 hover:-translate-y-1 transition-all group cursor-default relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <h3 class="text-3xl font-black text-text flex items-baseline gap-1">
            {{ serverStatus === 'ok' ? t('dashboard.normal') : t('dashboard.error') }} <span class="text-sm font-bold text-border-strong">API</span>
          </h3>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0 z-10">
      <!-- Activity Heatmap -->
      <div class="lg:col-span-2 bg-surface backdrop-blur-xl border border-border-strong rounded-3xl p-6 shadow-lg shadow-slate-200/40 flex flex-col relative overflow-hidden min-h-0">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-extrabold text-text flex items-center gap-2">
            ...
            <div class="w-full max-w-[40px] flex flex-col justify-end relative mb-3"
              ...
              >
                ...
                <div class="absolute -top-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 bg-surface text-white text-xs font-bold py-1.5 px-3 rounded-lg transition-all transform group-hover:-translate-y-1 whitespace-nowrap shadow-xl pointer-events-none z-20">
                  {{ val }} <span class="text-border-strong font-normal">Events</span>
                  <!-- little arrow -->
      ...
      <!-- Active Friends List -->
      <div class="bg-surface backdrop-blur-xl border border-border-strong rounded-3xl p-6 shadow-lg shadow-slate-200/40 flex flex-col min-h-0">
        <div class="flex items-center justify-between mb-6 shrink-0">
          <h3 class="text-xl font-extrabold text-text flex items-center gap-2">
            <div class="w-2 h-2 rounded-full bg-primary animate-pulse" />
            {{ t('dashboard.active_friends') }}
          </h3>
          <span class="text-xs font-bold text-border-strong">{{ onlineFriendsCount }} Online</span>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar space-y-1.5 pr-2 -mr-2">
          ...
          <div
            ...
            :key="friend.id"
            class="flex items-center gap-3 p-2.5 bg-surface border border-transparent hover:border-border-soft hover:shadow-sm rounded-2xl transition-all cursor-pointer group"
            @click="openPlayerProfile(friend)"
          >
            <div class="relative">
              <VrcAvatar
                :user="friend"
                custom-class="w-11 h-11 rounded-full object-cover shadow-sm bg-background/10 border border-border-soft"
              />
              <div class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-primary border border-border-soft" />
            </div>
            
            <div class="flex-1 min-w-0">
              <p class="text-sm font-extrabold text-text truncate group-hover:text-primary transition-colors">
                {{ friend.displayName }}
              </p>
              <p class="text-[10px] font-medium text-text-muted truncate flex items-center gap-1">
                <Map
            ...
            <div class="w-6 h-6 rounded-full bg-surface-hover flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <div class="w-1.5 h-1.5 rounded-full bg-surface" />
            </div>
          </div>
        </div>
      </div>
