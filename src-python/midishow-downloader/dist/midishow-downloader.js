...
exports.MidiShowDownloader = void 0;
...
class MidiShowDownloader {
    ...
    constructor() {
        ...
        this.requester = (0, axios_cookiejar_support_1.wrapper)(axios_1.default.create({
            headers: {
                "accept": "*/*",
    ...
    getCsrf(html) {
        const csrf = new jsdom_1.JSDOM(html)
            .window
            .document
    ...
    getMidiLink(html, id) {
        const link = new jsdom_1.JSDOM(html)
            .window
            .document
            .querySelector(`div[data-id="${id}"]`)
            ?.getAttribute('data-mid');
        if (!link)
            throw new Error('MIDI link not found');
        return link;
    }
    async login(identity, password, proxy = false) {
        const response = await this.requester.get('https://www.midishow.com/en/user/account/login', {
            headers: {
    ...
    async getMidiFile(id, proxy = false) {
        const pageResponse = await this.requester.get(`https://www.midishow.com/en/midi/${id}.html`, {
            proxy
        });
        const csrf = this.getCsrf(pageResponse.data);
        const response1 = await this.requester.post(`https://www.midishow.com/midi/new-file?id=${id}`, {
            'id': id
        }, {
            headers: {
                'X-CSRF-TOKEN': csrf,
        ...
        const response2 = await this.requester.get(this.getMidiLink(pageResponse.data, id)
...
exports.MidiShowDownloader = MidiShowDownloader;
