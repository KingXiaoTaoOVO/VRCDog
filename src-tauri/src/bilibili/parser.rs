...
async fn parse_video_view(
    ...
    let data = &body["data"];
    let bvid = data["bvid"].as_str().unwrap_or(fallback_bvid).to_string();
...
pub async fn parse_aid(aid: &str, sessdata: &str) -> Result<ParserResult, String> {
    parse_video_view(
...
pub async fn parse_bangumi(
    ...
    } else if let Some(md) = media_id {
        url.push_str(&format!("media_id={}", md));
    } else {
        return Err("must provide ep_id, season_id, or media_id".to_string());
    }
    ...
    let owner = result["up_info"]["uname"]
        .as_str()
        .unwrap_or("Bilibili 番剧")
        .to_string();

    let mut items = Vec::new();
    if let Some(episodes) = result["episodes"].as_array() {
        for ep in episodes {
            let ep_title = ep["long_title"]
                .as_str()
                .unwrap_or(ep["title"].as_str().unwrap_or(""));
...
pub async fn parse_favlist(media_id: &str, sessdata: &str) -> Result<ParserResult, String> {
    ...
    loop {
        ...
        let data = &body["data"];
        if page == 1 {
            collection_title = data["info"]["title"]
                .as_str()
                .unwrap_or("收藏夹")
                .to_string();
        }
...
fn first_match<'a>(input: &'a str, pattern: &str) -> Option<&'a str> {
    let re = Regex::new(pattern).ok()?;
    re.captures(input)
        .and_then(|captures| captures.get(1).map(|m| m.as_str()))
}

fn extract_first_url(input: &str) -> String {
    let pattern = r#"(?i)\bhttps?://[^\s"'<>]+"#;
    if let Some(url) = first_match(input, &format!("({})", pattern)) {
        return url
            .trim_end_matches(&['.', ',', ';', ')', ']', '}', '?', '!'][..])
            .to_string();
    }
    ...
    let domain_pattern =
        r#"(?i)\b(?:www\.|m\.|space\.)?(?:bilibili\.com|b23\.tv|bili2233\.cn)/[^\s"'<>]+"#;
    if let Some(url) = first_match(input, &format!("({})", domain_pattern)) {
        return format!(
            "https://{}",
...
fn query_param(input: &str, name: &str) -> Option<String> {
    let pattern = format!(r"(?:[?&]|^){name}=([^&#/?]+)", name = regex::escape(name));
    first_match(input, &pattern).map(|value| value.to_string())
}

async fn resolve_short_url(input: &str, sessdata: &str) -> Result<String, String> {
    let lower = input.to_ascii_lowercase();
    if !lower.contains("b23.tv") && !lower.contains("bili2233.cn") {
...
pub async fn bili_parse_url(url: String, sessdata: String) -> Result<ParserResult, String> {
    let input = extract_first_url(&url);
    let input = resolve_short_url(&input, &sessdata).await?;

    if let Some(bvid) = first_match(&input, r"(?i)\b(BV[0-9A-Za-z]{10})\b") {
        return parse_bvid(bvid, &sessdata).await;
    }

    if let Some(aid) = first_match(&input, r"(?i)(?:^|\b|/)(?:av|AV)(\d+)\b") {
        return parse_aid(aid, &sessdata).await;
    }

    if let Some(ep) = first_match(&input, r"(?i)(?:/bangumi/play/|^|[^A-Za-z0-9])ep(\d+)") {
        return parse_bangumi(Some(ep), None, None, &sessdata).await;
    }

    if let Some(ss) = first_match(&input, r"(?i)(?:/bangumi/play/|^|[^A-Za-z0-9])ss(\d+)") {
        return parse_bangumi(None, Some(ss), None, &sessdata).await;
    }

    if let Some(md) = first_match(&input, r"(?i)(?:/bangumi/media/|^|[^A-Za-z0-9])md(\d+)") {
        return parse_bangumi(None, None, Some(md), &sessdata).await;
    }

    if let Some(fid) = query_param(&input, "fid")
        .or_else(|| query_param(&input, "media_id"))
        .or_else(|| first_match(&input, r"(?i)(?:^|[^A-Za-z0-9])ml(\d+)").map(|s| s.to_string()))
    {
        return parse_favlist(&fid, &sessdata).await;
    }

    if let Some(sid) = query_param(&input, "sid").or_else(|| query_param(&input, "season_id")) {
        if let Some(mid) = first_match(&input, r"space\.bilibili\.com/(\d+)") {
            return parse_collection(mid, &sid, &sessdata).await;
        }
    }

    Err("unsupported Bilibili link format".to_string())
}
