use ab_glyph::{FontVec, Font, PxScale, ScaleFont, point};
...
impl VrUiRenderer {
                    ...
                    outlined.draw(|gx, gy, coverage| {
                        ...
                        if px >= 0 && py >= 0 && (px as u32) < width && (py as u32) < height {
                            ...
                            if idx + 3 < pixels.len() {
                                ...
                                let inv = 255 - alpha;
                                pixels[idx] = ((text_color[0] as u16 * alpha as u16 + pixels[idx] as u16 * inv as u16) / 255) as u8;
                                pixels[idx+1] = ((text_color[1] as u16 * alpha as u16 + pixels[idx+1] as u16 * inv as u16) / 255) as u8;
                                pixels[idx+2] = ((text_color[2] as u16 * alpha as u16 + pixels[idx+2] as u16 * inv as u16) / 255) as u8;
    ...
    pub fn render_vr_menu(
        ...
        let page_names = [
            "VrcDog 主菜单", "基础设置", "桌面投屏翻译", "OCR设置", "翻译服务", 
            "叠加层外观", "高级性能", "操作说明",
            "VRCX 社交状态", "VRCT 语音输入", "VRCLS 日志追踪", "OVRAS 空间控制"
        ];
        ...
        let items: Vec<String> = match page {
            ...
            1 => vec![
                format!("  主功能启用: {}", if translation_enabled { "开" } else { "关" }),
                format!("  原文与译文切换显示: {}", if config.dual_display { "开" } else { "关" }),
                format!("  手腕常驻显示: {}", if config.wrist_mode { "开" } else { "关" }),
                "  ◄ 返回主菜单".to_string(),
            ],
            2 => vec![
                format!("  桌面翻译模式: {}", if config.desktop_mode { "开" } else { "关" }),
                format!("  自动扫描: {}", if config.auto_scan_enabled { "开" } else { "关" }),
                format!("  扫描间隔: {}秒", config.auto_scan_interval),
                "  ◄ 返回上一页".to_string(),
            ],
            3 => vec![
                format!("  识别语言: {}", config.ocr_language),
                format!("  图像增强: {}", if config.ocr_image_enhance { "开" } else { "关" }),
            ...
            6 => vec![
                format!("  TTS语音播报: {}", if config.tts_enabled { "开" } else { "关" }),
                format!("  OSC聊天框输出: {}", if config.osc_chatbox_enabled { "开" } else { "关" }),
                format!("  扫描框状态: {}", if scan_active { "显示" } else { "隐藏" }),
                "  ◄ 返回上一页".to_string(),
            ],
            7 => vec![
                "  🎮 高级操作指南:".to_string(),
        ...
        Self::render_line_to_pixels(font, &header, scale, 12.0, cy, &mut pixels, w, h, [49,46,129]);
        cy = 48.0 + scaled.ascent();

        // Menu items
        for item in &items {
            Self::render_line_to_pixels(font, item, scale, 12.0, cy, &mut pixels, w, h, [67,56,202]);
            cy += line_h + 12.0;
        }

        // Footer
        cy = h as f32 - 22.0;
        Self::render_line_to_pixels(font, "扳机=确认  摇杆↕=选择  ↔=切页  B长按=关闭", ab_glyph::PxScale::from(16.0), 30.0, cy, &mut pixels, w, h, [156,163,175]);
    ...
    fn render_line_to_pixels(
        ...
        for ch in text.chars() {
            ...
            if cx + advance > width as f32 - 8.0 { break; }
                ...
                outlined.draw(|gx, gy, cov| {
                    ...
                    if px >= 0 && py >= 0 && (px as u32) < width && (py as u32) < height {
                        ...
                        if idx + 3 < pixels.len() {
                            ...
                            let inv = 255 - a;
                            pixels[idx] = ((color[0] as u16 * a as u16 + pixels[idx] as u16 * inv as u16) / 255) as u8;
                            pixels[idx+1] = ((color[1] as u16 * a as u16 + pixels[idx+1] as u16 * inv as u16) / 255) as u8;
                            pixels[idx+2] = ((color[2] as u16 * a as u16 + pixels[idx+2] as u16 * inv as u16) / 255) as u8;
