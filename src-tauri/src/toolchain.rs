...
fn find_in_drives(relative_paths: &[&str]) -> Option<String> {
    for drive in b'C'..=b'Z' {
        ...
        if !Path::new(&drive_path).exists() { continue; }
...
pub async fn check_system_status() -> AppResult<EnvironmentStatus> {
    // 1. Detect Unity Hub
    let mut hub_installed = is_registry_key_exists(HKEY_LOCAL_MACHINE, r#"SOFTWARE\Unity Technologies\Hub"#)
        || is_registry_key_exists(HKEY_LOCAL_MACHINE, r#"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Unity Hub"#);
    ...
    if let Some(loc) = get_registry_string(HKEY_CURRENT_USER, r#"SOFTWARE\Unity Technologies\Installer\Unity 2022.3.22f1"#, "Location x64") {
        if Path::new(&loc).join("Editor").join("Unity.exe").exists() {
            unity_installed = true;
        }
    }
    if !unity_installed {
        unity_installed = find_in_drives(&[r"Program Files\Unity\Hub\Editor\2022.3.22f1\Editor\Unity.exe"]).is_some();
    }

    // 3. Detect VCC / ALCOM
    let local_app_data = std::env::var("LOCALAPPDATA").unwrap_or_else(|_| "C:\\".to_string());
    
    let mut vcc_installed = is_registry_key_exists(HKEY_CURRENT_USER, r#"Software\VRChat\CreatorCompanion"#);
    if !vcc_installed {
        let vcc_paths = [
        ...
        if find_in_drives(&vcc_paths).is_some() || Path::new(&format!(r"{}\Programs\VRChat Creator Companion\CreatorCompanion.exe", local_app_data)).exists() {
            vcc_installed = true;
        }
    }
    ...
    let alcom_installed = find_in_drives(&alcom_paths).is_some() 
        ...
        || Path::new(&format!(r"{}\Programs\vrc-get-gui\vrc-get-gui.exe", local_app_data)).exists();
...
pub async fn uninstall_software(target: String) -> AppResult<()> {
    ...
    match target.as_str() {
        "hub" => {
            uninstall_paths.push(r#"C:\Program Files\Unity Hub\Uninstall Unity Hub.exe"#.to_string());
        },
        "unity" => {
            if let Some(p) = find_in_drives(&[r"Program Files\Unity\Hub\Editor\2022.3.22f1\Editor\Uninstall.exe"]) {
        ...
        "tool" | "vcc" | "alcom" => {
            ...
            uninstall_paths.push(format!(r"{}\Programs\VRChat Creator Companion\uninstall.exe", local_app_data));
            uninstall_paths.push(format!(r"{}\Programs\VRChat Creator Companion\unins000.exe", local_app_data));
            uninstall_paths.push(format!(r"{}\Programs\ALCOM\Uninstall ALCOM.exe", local_app_data));
            uninstall_paths.push(format!(r"{}\Programs\ALCOM\unins000.exe", local_app_data));
            uninstall_paths.push(format!(r"{}\Programs\vrc-get-gui\Uninstall VRChat Package Manager.exe", local_app_data));
            uninstall_paths.push(format!(r"{}\Programs\vrc-get-gui\unins000.exe", local_app_data));
            
            // Find across drives
            for drive in b'C'..=b'Z' {
                let drive_str = format!("{}:", drive as char);
                if !Path::new(&format!("{}\\", drive_str)).exists() { continue; }
                for p in &base_unins {
    ...
    for path in uninstall_paths {
        if Path::new(&path).exists() {
            let ps_cmd = format!("Start-Process -FilePath '{}' -ArgumentList '/S' -Verb RunAs -Wait", path);
...
pub async fn launch_software(target: String) -> AppResult<()> {
    ...
    let launch_path = match target.as_str() {
        "hub" => {
            ...
            if p.is_none() {
                if let Some(loc) = get_registry_string(HKEY_LOCAL_MACHINE, r#"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Unity Hub"#, "InstallLocation") {
                    let check = Path::new(&loc).join("Unity Hub.exe");
                    if check.exists() { p = Some(check.to_string_lossy().to_string()); }
                }
            }
        ...
        "unity" => {
            ...
            if p.is_none() {
                p = find_in_drives(&[r"Program Files\Unity\Hub\Editor\2022.3.22f1\Editor\Unity.exe"]);
            }
            p.unwrap_or_else(|| r#"C:\Program Files\Unity\Hub\Editor\2022.3.22f1\Editor\Unity.exe"#.to_string())
        },
        "tool" | "vcc" | "alcom" => {
            let mut found = None;
            if target == "alcom" || target == "tool" {
                found = find_in_drives(&[r"Program Files\ALCOM\ALCOM.exe", r"Programs\ALCOM\ALCOM.exe", r"ALCOM\ALCOM.exe", r"Programs\vrc-get-gui\vrc-get-gui.exe"])
                    .or_else(|| {
                        ...
                        if Path::new(&a1).exists() { Some(a1) } else if Path::new(&a2).exists() { Some(a2) } else { None }
                    });
            }
            if found.is_none() && (target == "vcc" || target == "tool") {
                found = find_in_drives(&[r"Program Files\CreatorCompanion\CreatorCompanion.exe", r"Programs\VRChat Creator Companion\CreatorCompanion.exe", r"CreatorCompanion\CreatorCompanion.exe"])
                    .or_else(|| {
                        let v = format!(r"{}\Programs\VRChat Creator Companion\CreatorCompanion.exe", local_app_data);
                        if Path::new(&v).exists() { Some(v) } else { None }
                    });
            }
    ...
    if Path::new(&launch_path).exists() {
        Command::new(&launch_path).spawn().map_err(|e| format!("启动失败: {}", e))?;
        Ok(())
    } else {
...
async fn download_file(
    ...
    let head_res = client.head(url).send().await.map_err(|e| format!("服务器连接失败: {}", e))?;
    if !head_res.status().is_success() {
        return Err(format!("下载请求被拒绝，状态码: {}", head_res.status()).into());
    }

    let total_size = head_res.headers().get(CONTENT_LENGTH)
        .and_then(|v| v.to_str().ok())
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(0);

    let supports_ranges = head_res.headers().get(ACCEPT_RANGES).map(|v| v == "bytes").unwrap_or(false);

    if total_size < 10 * 1024 * 1024 || !supports_ranges {
        return download_single_thread(app_handle, client, target, url, dest_path, total_size).await;
    }
    ...
    file.set_len(total_size).map_err(|e| format!("无法预分配磁盘空间: {}", e))?;
    ...
    let reporter_task = tokio::spawn(async move {
        ...
        loop {
            ...
            let _ = progress_app_handle.emit("install_progress", ProgressPayload {
    ...
    for i in 0..num_chunks {
        ...
        let task = tokio::spawn(async move {
            ...
            let mut retries = 3;
            loop {
                let req = client_c.get(&url_c).header(RANGE, format!("bytes={}-{}", start, end));
                    ...
                    if res.status().is_success() {
                        ...
                        while let Some(chunk_res) = stream.next().await {
                            match chunk_res {
                                ...
                                Err(_) => { chunk_ok = false; break; }
                            }
                        }
                        if chunk_ok && current_offset > end { break; }
                    }
                }
                retries -= 1;
                if retries == 0 { return Err(format!("无法下载区块: {}-{}", start, end)); }
    ...
    for task in tasks { task.await.map_err(|e| format!("线程崩溃: {}", e))??; }
    ...
    if actual_downloaded < total_size {
        return Err(format!("下载不完整：期望 {} 字节，实际只下载了 {} 字节", total_size, actual_downloaded).into());
    }
...
async fn download_single_thread(
    ...
    let res = client.get(url).send().await.map_err(|e| format!("下载失败: {}", e))?;
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        if total_size > 0 {
            ...
            if progress - last_progress >= 1.0 || downloaded == total_size {
                ...
                let _ = app_handle.emit("install_progress", ProgressPayload {
    ...
    if total_size > 0 && downloaded != total_size {
        ...
        return Err(format!("下载不完整：期望 {} 字节，实际 {} 字节", total_size, downloaded).into());
    }
...
fn register_unity_to_hub(app_handle: &AppHandle, unity_install_dir: &str, target: &str) {
    ...
    let parent_dir = Path::new(unity_install_dir).parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
    ...
    if hub_exe.exists() {
        let _ = Command::new(hub_exe).args(["--", "--headless", "install-path", "--set", unity_install_dir]).status();
    }
...
pub async fn install_software(
    ...
    if target == "ffmpeg" {
        ...
        if auto_delete { let _ = fs::remove_file(&dest_path); }
        ...
        match cmd.status() {
            Ok(exit_status) => {
                if !exit_status.success() {
                    ...
                    if code != 0 {
                        let unity_exe = Path::new(&unity_install_dir).join("Editor").join("Unity.exe");
                        if !unity_exe.exists() {
                            return Err(format!("Unity 安装程序退出码: {}，且未检测到安装文件", code).into());
                        }
                    }
                }
            }
